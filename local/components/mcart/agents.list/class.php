...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    final public function onPrepareComponentParams($arParams): array
    {
         ...
         */
        $arParams["ELEM_NUMBER"] == $arParams["ELEM_NUMBER"] ?? $arParams["ELEM_NUMBER"]["DEFAULT"];
        $arParams["CACHE_TIME"] == $arParams["CACHE_TIME"] ?? $arParams["CACHE_TIME"]["DEFAULT"];
    ...
    final public function executeComponent(): void
    {
         ...
         */ 
         $this->arResult['STAR_AGENTS'] = CUserOptions::GetOption("my_module", "favorites_agents");
        /*
    ...
    private static function getEntityDataClassById(array $arHlblock): string
    {
        ...
        $hlblock = Highloadblock\HighloadBlockTable::getById($ID)->fetch(); //где ID - id highloadblock блока в который будем добавлять элементы

        $entity_data_class = Highloadblock\HighloadBlockTable::compileEntity($hlblock);
        return $entity = $entity_data_class->getDataClass();
        ...
        private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        $nav->allowAllRecords(true)
            ->setPageSize($this->arParams['ELEM_NUMBER']) //Нужно передать параметр Количество элементов из мввсива $this->arParams
            ->initFromUri();
        ...
        while ($arAgent = $rsAgents->fetch()) {
            ...
            $fileId = $arAgent['UF_PHOTO'];
            if (isset($fileId)) {
                $file = \CFile::GetFileArray($fileId);
                $photoUrl = $file ? $file['SRC'] : '/local/components/mcart/agents.list/templates/.default/images/no-avatar.png';
                $arAgent['UF_PHOTO'] = $photoUrl;
            }
    ...
    public function clickStarAction($agentID)
    {
        ...
        $optionName = "favorites_agents";
        $defaultValue = [];
        $options = \CUserOptions::GetOption("my_module", $optionName, $defaultValue);

         // 2. Если значение есть, то

        if (!empty($options)) {
            $value = is_array($options) ? $options : [$options];
        ...
        \CUserOptions::SetOption("my_module", $optionName, $value);
